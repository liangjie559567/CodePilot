...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.info('[chat API] content length:', content.length, 'first 200 chars:', content.slice(0, 200));
    logger.info('[chat API] systemPromptAppend:', systemPromptAppend ? `${systemPromptAppend.length} chars` : 'none');
    ...
    if (files && files.length > 0) {
      ...
      fileMeta = files.map((f) => {
        ...
        if (buffer.length > MAX_FILE_SIZE) {
          throw new Error(`File ${f.name} exceeds maximum size of 10MB`);
        }
    ...
    // Attach code context if available
